...
import { useCreateBranch, useTenant } from '@/hooks/useTenants';
...
export default function NewBranchPage() {
  ...
  const {
    ...
    defaultValues: { isMainBranch: false, isOpen24h: false },
  });
  ...
  const onSubmit = async (raw: BranchForm) => {
    ...
    try {
      ...
      const newBranchId = result.data?.data?.id ?? result.data?.id;
      router.push(newBranchId ? `/tenants/${tenantId}/branches/${newBranchId}` : `/tenants/${tenantId}`);
    } catch {
      // error toast handled in useCreateBranch onError
    }
  };
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        action={
          <Button variant="outline" onClick={() => router.push(`/tenants/${tenantId}`)} className="gap-2 rounded-xl">
      ...
      <div className="section-glass">
        <div className="flex items-center gap-3 pb-4 mb-2 border-b border-white/10 dark:border-white/5">
          ...
          <div>
            <div className="font-semibold text-slate-800 dark:text-slate-200">New Branch Location</div>
            {tenant?.name && <div className="text-xs text-slate-400">{tenant.name}</div>}
          </div>
        </div>
